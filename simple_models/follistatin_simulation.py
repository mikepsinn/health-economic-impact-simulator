from muscle_mass_model import MuscleMassInterventionModel
from lifespan_model import LifespanImpactModel, generate_report as generate_lifespan_report
import os
...
def generate_follistatin_report(muscle_mass_lbs=2.0, lifespan_increase_pct=2.5, population_size=100000):
    ...
    lifespan_results = generate_lifespan_report(lifespan_increase_pct)
...
def save_follistatin_report(muscle_mass_lbs=2.0, lifespan_increase_pct=2.5, output_path='.'):
    ...
    # Generate filename
    filename = f'follistatin_m+{muscle_mass_lbs}lb_l{lifespan_increase_pct}pct.md'
    filepath = os.path.join(output_path, filename)
    
    # Generate and save the report
    report_content = generate_follistatin_report(muscle_mass_lbs, lifespan_increase_pct)
